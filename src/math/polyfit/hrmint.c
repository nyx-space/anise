...
/* Subroutine */ int hrmint_(integer *n, doublereal *xvals, doublereal *yvals,
  ...
  for (i__ = 1; i__ <= i__1; ++i__) {
    c1 = xvals[(i__2 = i__)] - *x;
    c2 = *x - xvals[(i__2 = i__ - 1)];
    denom = xvals[(i__2 = i__)] - xvals[(i__3 = i__ - 1)];
    ...
    this__ = prev + 1;
    next = this__ + 1;
    work[(i__2 = prev + (work_dim1 * 2) - work_offset)] =
        work[(i__3 = this__ + work_dim1 - work_offset)];
    ...
    work[(i__2 = this__ + (work_dim1 * 2) - work_offset)] =
        (work[(i__3 = next + work_dim1 - work_offset)] -
         work[(i__4 = prev + work_dim1 - work_offset)]) /
        denom;
    ...
    temp = work[(i__2 = this__ + work_dim1 - work_offset)] *
               (*x - xvals[(i__3 = i__ - 1)]) +
           work[(i__4 = prev + work_dim1 - work_offset)];
    work[(i__2 = this__ + work_dim1 - work_offset)] =
        (c1 * work[(i__3 = prev + work_dim1 - work_offset)] +
         c2 * work[(i__4 = next + work_dim1 - work_offset)]) /
        denom;
    work[(i__2 = prev + work_dim1 - work_offset)] = temp;
  }

  /*     The last column entries were not computed by the preceding loop; */
  /*     compute them now. */

  work[(i__1 = (*n * 2) - 1 + (work_dim1 * 2) - work_offset)] =
      work[(i__2 = (*n * 2) + work_dim1 - work_offset)];
  work[(i__1 = (*n * 2) - 1 + work_dim1 - work_offset)] =
      work[(i__2 = (*n * 2) + work_dim1 - work_offset)] *
          (*x - xvals[(i__3 = *n - 1)]) +
      work[(i__4 = (*n * 2) - 1 + work_dim1 - work_offset)];
  ...
  for (j = 2; j <= i__1; ++j) {
    ...
    for (i__ = 1; i__ <= i__2; ++i__) {
      ...
      xi = (i__ + 1) / 2;
      xij = (i__ + j + 1) / 2;
      c1 = xvals[(i__3 = xij - 1)] - *x;
      c2 = *x - xvals[(i__3 = xi - 1)];
      denom = xvals[(i__3 = xij - 1)] - xvals[(i__4 = xi - 1)];
      ...
      work[(i__3 = i__ + (work_dim1 * 2) - work_offset)] =
          (c1 * work[(i__4 = i__ + (work_dim1 * 2) - work_offset)] +
           c2 * work[(i__5 = i__ + 1 + (work_dim1 * 2) - work_offset)] +
           (work[(i__6 = i__ + 1 + work_dim1 - work_offset)] -
            work[(i__7 = i__ + work_dim1 - work_offset)])) /
          denom;

      /*           Compute the interpolated function value at X for the Ith */
      /*           interpolant. */

      work[(i__3 = i__ + work_dim1 - work_offset)] =
          (c1 * work[(i__4 = i__ + work_dim1 - work_offset)] +
           c2 * work[(i__5 = i__ + 1 + work_dim1 - work_offset)]) /
          denom;
    }
  }
  ...
  *f = work[(i__1 = work_dim1 + 1 - work_offset)];
