...
use anise::astro::orbit_geodetic::GeodeticOrbit;
use anise::file_mmap;
...
fn state_def_reciprocity() {
    ...
    assert_eq!(
        Orbit::cartesian(-2436.45, -2436.45, 6891.037, 5.088_611, -5.088_611, 0.0, epoch, eme2k),
        Orbit::keplerian(
...
fn geodetic_vallado() {
    ...
    let r = GeodeticOrbit::from_position(ri, rj, rk, epoch, eme2k);
...
fn with_init() {
    ...
    for ecc_incr in 0..100 {
        ...
        f64_eq!(
            new_state.ecc(),
            new_ecc,
            format!("wrong ecc: got {}\twanted {}", new_state.inc_deg(), new_ecc)
        );
    }
    for angle_incr in 0..360 {
        let new_aop = between_0_360(kep.aop_deg() + f64::from(angle_incr));
        let new_state = kep.add_aop_deg(f64::from(angle_incr));
        f64_eq!(
            new_state.aop_deg(),
            new_aop,
    ...
    for angle_incr in 0..360 {
        ...
        let new_state = kep.add_raan_deg(f64::from(angle_incr));
        f64_eq!(
            new_state.raan_deg(),
            new_raan,
            format!(
    ...
    for angle_incr in 0..360 {
        ...
        let new_state = kep.with_ta_deg(new_ta);
        f64_eq!(
            new_state.ta_deg(),
            new_ta,
    ...
    for angle_incr in 0..360 {
        ...
        let new_state = kep.add_inc_deg(f64::from(angle_incr));
        f64_eq!(
            new_state.inc_deg(),
            new_inc,
