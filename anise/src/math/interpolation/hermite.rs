...
use crate::math::interpolation::{CorruptedDataSnafu, NotEnoughSamplesSnafu, TooManySamplesSnafu};
...
pub fn hermite_eval<Y: StridedDataAccess, YP: StridedDataAccess>(
    ...
    let n = xs.len();
        ...
        n == ys.len() && n == yps.len(),
        CorruptedDataSnafu {
            what: "lengths of xs, ys, and yps must be equal and non-zero"
        }
    );
    ...
    for i in 1..=n - 1 {
        ...
        work[prev_work_idx + 1 + 2 * n - 1] =
            (work[curr_work_idx] - work[prev_work_idx - 1]) / denom;
    ...
    for j in 2..=(2 * n) - 1 {
        for i in 1..=(2 * n) - j {
            ...
            work[(i - 1) + 2 * n] =
                (c1 * deriv_val_i_minus_1 + c2 * deriv_val_i + (func_val_i - func_val_i_minus_1))
                    / denom;
...
mod hermite_ut {
    ...
    impl StridedDataAccess for MockStridedData {
        fn get(&self, index: usize) -> f64 {
            if index >= self.logical_len {
                panic!(
                    "MockStridedData: Access out of bounds. Index: {}, Logical Length: {}",
    ...
    fn test_hermite_eval_with_mock_strided_data_spice_example() {
        ...
        let x_eval = 2.0;
        let (y_eval_simple, yp_eval_simple) =
            hermite_eval(&ts, &mock_ys_simple, &mock_yps_simple, x_eval).unwrap();

        assert!(
            (y_eval_simple - 141.0).abs() < f64::EPSILON,
            "Simple Mock Y error. Expected 141.0, got {}",
            y_eval_simple
        );
        assert!(
            (yp_eval_simple - 456.0).abs() < f64::EPSILON,
            "Simple Mock YP error. Expected 456.0, got {}",
            yp_eval_simple
        );
        ...
        let (y_eval_strided, yp_eval_strided) =
            hermite_eval(&ts, &mock_ys_strided, &mock_yps_strided, x_eval).unwrap();

        assert!(
            (y_eval_strided - 141.0).abs() < f64::EPSILON,
            "Strided Mock Y error. Expected 141.0, got {}",
            y_eval_strided
        );
        assert!(
            (yp_eval_strided - 456.0).abs() < f64::EPSILON,
            "Strided Mock YP error. Expected 456.0, got {}",
            yp_eval_strided
        );
    }
    ...
    fn test_hermite_eval_with_mock_strided_data_simple_linear() {
        ...
        assert!(
            (y_eval - 5.0).abs() < f64::EPSILON,
            "Expected y_eval = 5.0, got {}",
            y_eval
        );
        assert!(
            (yp_eval - 1.0).abs() < f64::EPSILON,
            "Expected yp_eval = 1.0, got {}",
            yp_eval
        );
        ...
        assert!(
            (y_eval_2 - 2.0).abs() < f64::EPSILON,
            "Expected y_eval_2 = 2.0, got {}",
            y_eval_2
        );
        assert!(
            (yp_eval_2 - 1.0).abs() < f64::EPSILON,
            "Expected yp_eval_2 = 1.0, got {}",
            yp_eval_2
        );
    }
}
