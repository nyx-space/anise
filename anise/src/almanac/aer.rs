...
impl Almanac {
    ...
    pub fn azimuth_elevation_range_sez(&self, rx: Orbit, tx: Orbit) -> AlmanacResult<AzElRange> {
        ...
        // Compute the SEZ DCM
        let from = tx.frame.orientation_id * 100_000 + 011;
...
mod ut_aer {
    ...
    fn verif_edge_case() {
        ...
        let ground_station = Orbit::try_from_latlongalt(
            latitude_deg,
            longitude_deg,
            height_km,
    ...
    fn gmat_validation() {
        ...
        let almanac = MetaAlmanac::new("../data/ci_config.dhall".to_string())
            .unwrap()
            .process()
            .unwrap();
        let itrf93 = almanac.frame_from_uid(EARTH_ITRF93).unwrap();
        ...
        for (sno, state) in states.iter().enumerate() {
            ...
            let madrid = Orbit::try_from_latlongalt(
                latitude_deg,
                longitude_deg,
                height_km,
                MEAN_EARTH_ANGULAR_VELOCITY_DEG_S,
            ...
            println!("err = {:e}", aer.range_km - expect);
        }

        /*
                [Earth J2000] 2023-11-16T13:35:30.231999909 UTC	position = [58643.769881020, -61696.430010747, -36178.742480219] km	velocity = [2.148654262, -1.202488371, -0.714016096] km/s
        range difference 2.8608524933224544e-1	 range rate difference: 2.5518327089837856e-1
        [Earth J2000] 2023-11-16T14:41:30.231999930 UTC	position = [66932.786922851, -66232.181345574, -38873.607459037] km	velocity = [2.040554622, -1.092315772, -0.649375769] km/s
        range difference 1.524116205982864e0	 range rate difference: 1.9537874827668378e-1
        [Earth J2000] 2023-11-16T15:40:30.231999839 UTC	position = [74004.678508956, -69951.392953800, -41085.743778595] km	velocity = [1.956605843, -1.011238479, -0.601766262] km/s
        range difference 2.639831733889878e0	 range rate difference: 1.2757790073978081e-1
        [Earth J2000] 2023-11-16T16:39:30.232000062 UTC	position = [80796.571971532, -73405.942333285, -43142.412981359] km	velocity = [1.882014733, -0.942231959, -0.561216138] km/s
        range difference 3.5948990078322822e0	 range rate difference: 5.04787865334162e-2
        [Earth J2000] 2023-11-16T18:18:30.231999937 UTC	position = [91643.443331668, -78707.208988294, -46302.221669744] km	velocity = [1.773134524, -0.846263432, -0.504774983] km/s
        range difference 4.448131133962306e0	 range rate difference: 8.57852817148057e-2
