...
pub struct Mass {
    ...
    pub dry_mass_kg: f64,
    /// Usable propellant mass of the spacecraft in kg
    pub usable_prop_mass_kg: f64,
    /// Unusable propellant mass of the spacecraft in kg
    pub unusable_prop_mass_kg: f64,
}
...
impl Mass {
    ...
    pub fn from_dry_and_fuel_masses(dry_mass_kg: f64, fuel_mass_kg: f64) -> Self {
        Self {
            dry_mass_kg,
            usable_prop_mass_kg: fuel_mass_kg,
            unusable_prop_mass_kg: 0.0,
        }
    }
    /// Returns the total mass in kg
    pub fn total_mass_kg(&self) -> f64 {
        self.dry_mass_kg + self.usable_prop_mass_kg + self.unusable_prop_mass_kg
    }
}
...
impl Encode for Mass {
    fn encoded_len(&self) -> der::Result<der::Length> {
        self.dry_mass_kg.encoded_len()?
            + self.usable_prop_mass_kg.encoded_len()?
            + self.unusable_prop_mass_kg.encoded_len()?
    }

    fn encode(&self, encoder: &mut impl Writer) -> der::Result<()> {
        self.dry_mass_kg.encode(encoder)?;
        self.usable_prop_mass_kg.encode(encoder)?;
        self.unusable_prop_mass_kg.encode(encoder)
    }
}
...
impl<'a> Decode<'a> for Mass {
    fn decode<R: Reader<'a>>(decoder: &mut R) -> der::Result<Self> {
        Ok(Self {
            ...
            usable_prop_mass_kg: decoder.decode()?,
            unusable_prop_mass_kg: decoder.decode()?,
        })
    }
}
...
mod mass_ut {
    ...
    fn example_repr() {
        let repr = Mass {
            dry_mass_kg: 50.0,
            usable_prop_mass_kg: 15.7,
            unusable_prop_mass_kg: 0.3,
        };
